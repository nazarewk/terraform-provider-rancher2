...
func (c *Config) SetUserPasswordByName(username, pass string) (string, *managementClient.User, error) {
	if len(username) == 0 {
		return "", nil, fmt.Errorf("[ERROR] Setting user password: Username is nil")
	}
	...
	newUser, err := c.SetUserPassword(user, pass)

	return pass, newUser, err
}

func (c *Config) SetUserPassword(user *managementClient.User, pass string) (*managementClient.User, error) {
	if !c.UserPasswordChanged(user, pass) {
		return user, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("[ERROR] Setting %s password: %s", user.Username, err)
	}
